...
const GoldPage = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/api/chart-data?range=${range}`)
      .then((res) => {
        ...
        const historicalPrices = sorted.map((d) =>
          d.type === "Historical" ? d.price : null
        );
        const predictedPrices = sorted.map((d) =>
          d.type === "Predicted" ? d.price : null
        );
