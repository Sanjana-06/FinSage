...
if __name__ == "__main__":
    try:
        user_income = float(input("Enter your annual income (in INR): "))
        risk_capacity = input("Enter your risk capacity (low, medium, or high): ").strip().lower()
        investment_duration = int(input("Enter your investment duration (in years): "))

        if risk_capacity not in ['low', 'medium', 'high']:
            raise ValueError("Invalid risk capacity. Choose from 'low', 'medium', or 'high'.")

        recommended_allocation = investment_allocation(user_income, risk_capacity, investment_duration)
        print("\nRecommended Investment Allocation:")
        for asset, allocation in recommended_allocation.items():
            print(f"{asset}: {allocation * 100}%")
